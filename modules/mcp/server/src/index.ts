...
// For compare_to_reference: emit the screenshot + reference as two image blocks
// the model can diff, then the remaining meta (url, viewport, note, checklist) as text.
...
server.tool(
  ...
  ({ id, content, contentFields, seo, autoResponsive }) =>
    run(() =>
      api.put(`/api/mcp/v1/pages/${id}/content`, { content, contentFields, seo, autoResponsive })
    )
)
