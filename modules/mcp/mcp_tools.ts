 ...
 * - `uploadMedia({ path?, url?, purpose?, alt?, title? })` — a multipart upload.
 *   The transport fetches a `url` itself, enforcing the placeholder/SSRF guards,
 *   and stamps provenance (origin + sourceUrl) before posting the bytes.
 *
...
export interface ToolDeps {
  ...
  uploadMedia: (source: UploadSource) => Promise<unknown>
}

/**
 * Server-level instructions handed to the client on connect — the workflow the
 * catalog can't enforce. Kept here so the in-app transport uses it directly;
 * MIRRORED verbatim in `server/src/index.ts` for the stdio transport (keep in
 * sync). This is where "translate a design faithfully" is spelled out: match the
 * palette, use the design's own images, check coverage, look before publishing.
 */
...
export function registerTools(
  ...
  server.tool(
    ...
    { key: z.string() },
    ({ key }) => run(() => call('DELETE', `/api/mcp/v1/collections/${key}`))
  )
  server.tool(
    ...
    {},
    () => run(() => call('GET', '/api/mcp/v1/collections/trashed'))
  )
  server.tool(
    'restore_collection',
    'Restore a trashed collection (undo delete_collection).',
    { key: z.string() },
    ({ key }) => run(() => call('POST', `/api/mcp/v1/collections/${key}/restore`))
  )
  server.tool(
    ...
    { key: z.string() },
    ({ key }) => run(() => call('DELETE', `/api/mcp/v1/collections/${key}/force`))
  )
  server.tool(
    'add_field',
  ...
  server.tool(
    ...
    ({ id, brief }) => run(() => call('PUT', `/api/mcp/v1/pages/${id}/brief`, { brief }))
  )
  server.tool(
    'check_design_coverage',
  ...
  server.tool(
    ...
    ({ id, frame, mode, viewportWidth }) =>
      run(() =>
        call('POST', `/api/mcp/v1/pages/${id}/analyze-layout`, { frame, mode, viewportWidth })
      )
  )
  server.tool(
    'lint_layout',
    ...
    { id: z.string(), viewport: z.enum(['desktop', 'tablet', 'mobile']).default('desktop') },
    ({ id, viewport }) =>
      run(() => call('GET', `/api/mcp/v1/pages/${id}/lint-layout?viewport=${viewport}`))
  )
  const PatchOps = z
    .array(
      z.object({
        ...
        index: z
          .number()
          .optional()
          .describe('Position within the target array (default: append).'),
      })
    )
  ...
  server.tool(
    ...
    ({ id, presetId, parentId, slot, index }) =>
      run(() =>
        call('POST', `/api/mcp/v1/pages/${id}/insert-section`, { presetId, parentId, slot, index })
      )
  )
  server.tool(
    'publish_page',
  ...
  server.tool('delete_template', 'Delete a template by id.', { id: z.string() }, ({ id }) =>
    run(() => call('DELETE', `/api/mcp/v1/templates/${id}`))
  )
  server.tool(
    'update_template',
    'Update a template.',
  ...
  server.tool(
    ...
    {},
    () => run(() => call('GET', '/api/mcp/v1/forms'))
  )
  server.tool(
    'get_form',
    'Get one form definition with its full `fields` schema.',
    { id: z.string() },
    ({ id }) => run(() => call('GET', `/api/mcp/v1/forms/${id}`))
  )
  server.tool(
    'create_form',
    ...
    (args) => run(() => call('POST', '/api/mcp/v1/forms', args))
  )
  server.tool(
    'update_form',
  ...
  server.tool(
    ...
    {},
    () => run(() => call('GET', '/api/mcp/v1/appearance'))
  )
  server.tool(
    'set_appearance',
  ...
  server.tool(
    ...
    {},
    () => run(() => call('GET', '/api/mcp/v1/content-paths'))
  )
  server.tool(
    'set_content_paths',
    ...
    (args) => run(() => call('PUT', '/api/mcp/v1/content-paths', args))
  )
  ...
  server.tool(
    ...
    ({ page, pageSize, search, origin }) => {
      ...
      const qs = q.toString()
      return run(() => call('GET', `/api/mcp/v1/media${qs ? `?${qs}` : ''}`))
    }
  )
  server.tool(
    'upload_media',
    ...
    ({ path, url, purpose, alt, title }) =>
      run(() => uploadMedia({ path, url, purpose, alt, title }))
  )
  server.tool(
    'crop_media',
    ...
    ({ mediaId, x, y, width, height, targetWidth, alt, title }) =>
      run(() =>
        call('POST', `/api/mcp/v1/media/${mediaId}/crop`, {
          x,
          y,
          width,
          height,
  ...
  server.tool(
    ...
    ({ id, alt, title, description }) =>
      run(() => call('PATCH', `/api/mcp/v1/media/${id}`, { alt, title, description }))
  )
  ...
  server.tool(
    ...
    (args) => {
      ...
      const qs = q.toString()
      return run(() => call('GET', `/api/mcp/v1/products${qs ? `?${qs}` : ''}`))
    }
  )
  server.tool(
    'get_product',
    ...
    { id: z.string() },
    ({ id }) => run(() => call('GET', `/api/mcp/v1/products/${id}`))
  )
  server.tool(
    'create_product',
    ...
    (args) => run(() => call('POST', '/api/mcp/v1/products', args))
  )
  server.tool(
    'update_product',
    ...
    ({ id, ...body }) => run(() => call('PUT', `/api/mcp/v1/products/${id}`, body))
  )
  server.tool(
    'delete_product',
    'Delete (archive) a product. Soft delete — orders that reference it are preserved.',
    { id: z.string() },
    ({ id }) => run(() => call('DELETE', `/api/mcp/v1/products/${id}`))
  )
  server.tool(
    'add_variant',
    ...
    ({ productId, ...body }) =>
      run(() => call('POST', `/api/mcp/v1/products/${productId}/variants`, body))
  )
  server.tool(
    'update_variant',
  ...
  const contentOptional = {
    ...
    password: z
      ...
      .optional()
      .describe(
        'Plaintext gate password — REQUIRED when visibility is PROTECTED; stored encrypted.'
      ),
    featuredImage: z
      .string()
      .nullable()
  ...
  server.tool('delete_content', 'Delete (trash) a content post.', { id: z.string() }, ({ id }) =>
    run(() => call('DELETE', `/api/mcp/v1/content/${id}`))
  )
