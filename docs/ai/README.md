...
| [cms-content-modeling.md](./cms-content-modeling.md) | Field-type catalog (incl. `SELECT`/`MULTISELECT` with `Label : value` options + key auto-fill), single types, relations (4 cardinalities + public id→label resolution), components (inline + registry), per-field width                                                                                                                                                                            |
| [content-taxonomy-and-visibility.md](./content-taxonomy-and-visibility.md) | Content **Categories** (hierarchical) + **Tags** (flat), `/category/:slug` + `/tag/:slug` archives, per-post **visibility** (Public/Protected/Member, server-side gate + `dl_unlocked` cookie), archive "Use as page" overrides                                                                                                                                                                            |
...
| [forms.md](./forms.md)                               | Custom forms: named forms with a JSON field schema, per-form validation (whitelist + 422) on the shared submit pipeline, FormBlock auto-render + 12-col widths, secure magic-byte file uploads, the submissions inbox                                                                                                    |
...
| [templates.md](./templates.md)           | Reusable HEADER/FOOTER/LAYOUT/COMPONENT/EMAIL/COLLECTION templates + per-page composition; **Trash & restore** for Templates and Menus                                             |
...
| [custom-templates.md](./custom-templates.md) | Coded page "kits" — self-contained folders (pages, file-pages, code chrome, collection & email templates); **fail-closed activation toggle** (`template_kits`); reading collections at runtime (`useCollectionRecords`) and using the store via `/api/shop/*`            |
...
| Doc                                          | Topics                                                                                                                                                     |
| -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [changelog-2026-09.md](./changelog-2026-09.md) | 2026-09 `develop` batch: CMS multi-select, Content taxonomies + visibility, e-commerce product tags + `/shop/` archives, template-kit activation, Templates/Menus trash, standardised list URL params |
