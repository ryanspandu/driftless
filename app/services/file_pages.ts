...
/** The file-page registered at this URL path (no leading slash; `""` = home), or undefined. */
export function findFilePageByPath(path: string): FilePage | undefined {
  return FILE_PAGES.find((p) => p.path === path)
}
 ...
 * page wins, so listing the shadowed file-page too would be confusing). The id
 * is synthetic (`file:<kit>/<file>`) and `source: 'file'` marks it read-only.
 */
export function fileSummaries(excludePaths: Set<string>): PageSummaryDto[] {
  return FILE_PAGES.filter((fp) => !excludePaths.has(fp.path)).map((fp) => ({
