...
export function readComponentFields(config: Record<string, unknown>): ComponentSubField[] {
...
export function componentSchemaError(config: Record<string, unknown>): string | null {
...
export function ComponentSchemaEditor({
  ...
  const setFields = (next: ComponentSubField[]) => onChange({ ...config, fields: next })
  ...
  return (
    <div className="space-y-4 rounded-lg border border-border/80 bg-muted/35 p-4">
      <div className="flex items-center justify-between gap-4">
        <p className="text-xs font-medium text-muted-foreground">Component fields</p>
        {showRepeatable ? (
          <label className="flex cursor-pointer items-center gap-2 text-sm">
            <Checkbox
              checked={repeatable}
              disabled={disabled}
              onCheckedChange={(v) => onChange({ ...config, repeatable: v === true })}
            />
            Repeatable
          </label>
        ) : null}
            ...
            >
              <Input
                ...
                onChange={(e) => {
                  ...
                  // Key follows the label until the user edits the key directly.
                  // Detect "not yet edited" by the key still matching the slug of
                  // the CURRENT label (or being empty) — the old `f.key ? …`
                  // check froze the key at its first character.
                  const linked = !f.key || f.key === slugifyKey(f.label)
                  patchField(i, linked ? { label, key: slugifyKey(label) } : { label })
                }}
              />
              <Input
                value={f.key}
